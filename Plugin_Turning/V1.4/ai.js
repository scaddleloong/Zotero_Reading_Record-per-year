...
Date: 2026/06/24
ai.js V1.3: structured AI paper analysis (9 fields) + streaming AI_QA (SSE)
...
// Streaming AI call (Anthropic + OpenAI-compatible), SSE parser
// onChunk(deltaText) is called per text increment; options: { maxTokens, signal }
// Returns { ok:true, text } | { ok:false, error } | { aborted:true } — never throws
async function callAIStream(userPrompt, systemPrompt, onChunk, options) {
    var opts = options || {};
    var cfg = getAPIConfig();
    if (!cfg.endpoint || !cfg.apiKey) {
    ...
    var isAnthropic = _isAnthropic(cfg.endpoint);
    ...
    try {
        ...
        if (!resp.ok) {
            ...
            return { ok: false, error: _loc("ai.error.http", resp.status) + errDetail };
        }
        if (!resp.body || !resp.body.getReader) {
            return { ok: false, error: _loc("ai.error.response") };
        }
        ...
        function handlePayload(payload) {
            ...
            var data = null;
            try { data = JSON.parse(payload); } catch (e) { return null; }
            if (isAnthropic) {
                ...
                if (data.choices && data.choices.length > 0) {
                    ...
                    if (data.choices[0].finish_reason === "stop") return "done";
                }
            }
            return null;
        }
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (line.indexOf("data:") === 0) {
                    ...
                    if (!payload) continue;
                    var st = handlePayload(payload);
                    if (st === "done") { terminated = true; break; }
                    if (st === "error") {
                        return { ok: false, error: _loc("ai.error.response") };
                    }
                }
        ...
        rawBody += decoder.decode(); // flush trailing multibyte chars

        // Fallback: server ignored stream:true and returned full JSON
        if (!receivedText) {
            try {
                var data = JSON.parse(rawBody.trim());
                var text = "";
                if (isAnthropic) {
                    if (data.content && data.content.length > 0) text = data.content[0].text;
                } else {
                    if (data.choices && data.choices.length > 0) text = data.choices[0].message.content;
                }
                if (text) {
                    onChunk(text);
                    full = text;
        ...
        if (receivedText) return { ok: true, text: full };
        return { ok: false, error: _loc("ai.error.response") };
    } catch (e) {
        if (e && e.name === "AbortError") return { aborted: true };
        Zotero.debug("callAIStream error: " + e);
        return { ok: false, error: _loc("ai.error.generic") };
    }
}
