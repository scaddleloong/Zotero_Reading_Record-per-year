...
Date: 2026/06/24
config.js V1.1: localized settings UI, _loc() in settings window
...
function _generateSettingsHTML() {
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>${_loc("settings.title")}</title>
<style>
...
<body>
<h2>${_loc("settings.title")}</h2>
<p class="sub">${_loc("settings.subtitle")}</p>

<div class="field">
  <label for="endpoint">${_loc("settings.endpoint.label")}</label>
  <input type="url" id="endpoint" value="${cfg.endpoint}" placeholder="https://api.deepseek.com" spellcheck="false"/>
  <div class="hint">${_loc("settings.endpoint.hint", "<code>https://api.deepseek.com</code>, <code>https://api.openai.com</code>, <code>https://api.anthropic.com</code>")}</div>
</div>

<div class="field">
  <label for="apiKey">${_loc("settings.apikey.label")}</label>
  <input type="password" id="apiKey" value="${cfg.apiKey}" placeholder="sk-..." spellcheck="false"/>
  <div class="hint">${_loc("settings.apikey.hint")}</div>
</div>

<div class="field">
  <label for="model">${_loc("settings.model.label")}</label>
  <input type="text" id="model" value="${cfg.model}" placeholder="e.g. deepseek-chat, gpt-4o, claude-sonnet-4-6" spellcheck="false"/>
  <div class="hint">${_loc("settings.model.hint")}</div>
</div>

<div class="btn-row">
  <button class="btn" id="btn-test">${_loc("settings.btn.test")}</button>
  <button class="btn primary" id="btn-save">${_loc("settings.btn.save")}</button>
...
document.getElementById('btn-save').addEventListener('click', function() {
  if (typeof window._saveAPIConfig !== 'function') {
    setStatus(_loc('settings.error.save'), 'err');
    return;
  }
  ...
  setStatus(_loc('settings.saved'), 'ok');
  clearTimeout(_to);
...
document.getElementById('btn-test').addEventListener('click', async function() {
  ...
  if (!endpoint) { setStatus(_loc('settings.api.empty.endpoint'), 'err'); return; }
  if (!apiKey)   { setStatus(_loc('settings.api.empty.key'), 'err'); return; }

  if (typeof window._testAPIConnection !== 'function') {
    setStatus(_loc('settings.error.test'), 'err');
    return;
  }
  ...
  try {
    ...
    if (result.ok) {
      setStatus(_loc('settings.test.ok'), 'ok');
    } else if (result.status === 0) {
      setStatus(_loc('settings.test.error.url'), 'err');
    } else if (result.status === 401 || result.status === 403) {
      setStatus(_loc('settings.test.error.auth'), 'err');
    } else {
      setStatus(_loc('settings.test.error.http', result.status), 'err');
    }
  } catch (e) {
    setStatus(_loc('settings.test.error.unexpected', e.message || e), 'err');
  }
