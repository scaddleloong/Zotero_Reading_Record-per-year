...
<div class="year-nav" id="year-nav">
    ...
    <button class="settings-btn" id="settings-btn" title="${_loc("heatmap.chart.legend.ann")}">&#x2699;&#xFE0F;</button>
</div>

<!-- Statistics -->
<div class="stats" id="stats">
    <h2><span id="stat-title">${_loc("heatmap.stats.title", defaultYear)}</span></h2>
    <div id="stat-total">${_loc("heatmap.stats.total", 0)}</div>
    <div id="stat-days">${_loc("heatmap.stats.days", 0)}</div>
    <div id="stat-avg">${_loc("heatmap.stats.avg", "0.0")}</div>
</div>
...
<div class="legend">
    <span>${_loc("heatmap.legend.less")}</span>
    <div class="legend-box level-0"></div>
    <div class="legend-box level-1"></div>
    <div class="legend-box level-2"></div>
    <div class="legend-box level-3"></div>
    <div class="legend-box level-4"></div>
    <span>${_loc("heatmap.legend.more")}</span>
</div>

<p style="margin-top:15px;font-size:12px;color:#666;">
${_loc("heatmap.hint.click")}
</p>
...
${_loc("heatmap.chart.hint")}
</p>

<!-- Detail Panel -->
<div id="detail-panel">
    <div id="detail-header">
        <h3>${_loc("heatmap.detail.title","")} <span id="detail-date"></span></h3>
...
function renderYear(year) {
    ...
    document.getElementById('stat-title').textContent = _loc("heatmap.stats.title", year);
    document.getElementById('stat-total').textContent = _loc("heatmap.stats.total", totalHours);
    document.getElementById('stat-days').textContent = _loc("heatmap.stats.days", activeDays);
    document.getElementById('stat-avg').textContent = _loc("heatmap.stats.avg", avgMinutes);
    ...
    while (cur <= endDate) {
        ...
        cell.title = iso + ' | ' + minutes.toFixed(1) + ' ' + _loc("heatmap.detail.minutes");
...
function renderTrendChart(year, gridWidth) {
    ...
    if (!hasData) {
        ...
        emptyMsg.textContent = _loc("heatmap.chart.empty", year);
    ...
    titleL.textContent = _loc("heatmap.chart.axis.time");
    ...
    titleR.textContent = _loc("heatmap.chart.axis.ann");
    ...
    legL.textContent = _loc("heatmap.chart.legend.reading");
    ...
    legR.textContent = _loc("heatmap.chart.legend.ann");
    ...
    totalLabel.textContent = _loc("heatmap.chart.total", totH, cumAnns);
    ...
    for (i = 0; i < points.length; i++) {
        (function(el) {
            el.addEventListener('mouseenter', function(e) {
                ...
                tip.innerHTML = '📅 ' + date + '<br>📖 ' + _loc("heatmap.chart.total", (cumMin / 60).toFixed(1), cumAnn);
    ...
    // Click delegation for SVG chart points (more reliable than per-element listeners)
    container.addEventListener('click', function(e) {
        var el = e.target;
        while (el && el !== container) {
            var date = el.getAttribute ? el.getAttribute('data-date') : null;
            if (date) {
                showDetail(date);
                return;
            }
...
function showDetail(date) {
    ...
    document.getElementById('detail-time').textContent = minutes.toFixed(1) + ' ' + _loc("heatmap.detail.minutes");

    var content = document.getElementById('detail-content');

    if (items.length === 0) {
        content.innerHTML = '<p class="no-data">' + _loc("heatmap.detail.nodata") + '</p>';
    } else {
        var html = '';
        for (var i = 0; i < items.length; i++) {
            ...
            html += '<div class="ai-panel" id="ais-' + escapeAttr(item.key) + '" style="display:none"><div class="ai-loading">' + _loc("heatmap.ai.loading") + '</div><div class="ai-body"></div><div class="ai-error" style="display:none"></div></div>';

            // child notes
            if (item.notes && item.notes.length > 0) {
                html += '<div class="section-label">' + _loc("heatmap.detail.section.notes") + '</div>';
            ...
            if (item.annotations && item.annotations.length > 0) {
                html += '<div class="section-label">' + _loc("heatmap.detail.section.ann") + '</div>';
                for (var j = 0; j < item.annotations.length; j++) {
                    ...
                    if (ann.type === 'highlight') {
                        ...
                        html += '<div class="ann-comment">' + _loc("heatmap.detail.sticky") + ': ' + escapeHtml(ann.comment) + '</div>';
                    } else {
                        if (ann.text) {
            ...
            if ((!item.notes || item.notes.length === 0) && (!item.annotations || item.annotations.length === 0)) {
                html += '<div style="margin-left:16px;font-size:12px;color:#8b949e;">' + _loc("heatmap.detail.nomarks") + '</div>';
            }
            ...
            (function(el) {
                el.addEventListener('click', async function() {
                    ...
                    if (typeof window._generateAISummary !== 'function') {
                        ...
                        error.textContent = _loc("heatmap.ai.unavailable");
                    ...
                    try {
                        ...
                        loading.style.display = 'none';
                        error.textContent = _loc("ai.error.runtime", e.message || e);
                        error.style.display = 'block';
                    }
                });
            })(titles[k]);
